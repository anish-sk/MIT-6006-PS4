...
class TransactionLog(AVL):
    def __init__(self, item = None, parent = None):
        ...
        self.Tmin=0
        if(self.item):
            self.Dsum=self.item.d
            self.Tmax=self.item.key
            self.Tmin=self.item.key
    ...
    def _update(self):
        ...
        self.Tmax=self.item.key
        if(self.left):
            self.Dsum+=self.left.Dsum
            self.Tmin=self.left.Tmin
        if(self.right):
            self.Dsum+=self.right.Dsum
            self.Tmax=self.right.Tmax
    ...
    def interval_revenue(self, t1, t2):
        ...
        # TODO: Implement me #
        ######################
        if(self.Tmin>=t1 and self.Tmax<=t2):
            return self.Dsum
        if(self.Tmin>t2 or self.Tmax<t1):
            return 0
        ans=0
        if(self.item.key>=t1 and self.item.key<=t2):
            ans+=self.item.d
        if(self.left):
            ans+=self.left.interval_revenue(t1,t2)
        if(self.right):
            ans+=self.right.interval_revenue(t1,t2)
        return ans
